...
public class MainActivity extends FragmentActivity {
    ...
    private void getDataFromServer() throws IOException, JSONException {
        Log.i("LOCAL", "Data received from the server");
    ...
    private void getFromLocalDb() {
        if (!entityDao.getAll().isEmpty()) {
            Log.i("LOCAL", "Data received from local database");
            allEntities = entityDao.getAll();
            ...
            isEmpty = true;
            Log.i("LOCAL", "Data do not received from local database or from the server");
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (isConected || !isEmpty) {
            if (isConected)
                text.setText("Data received from remote server");
            else
                text.setText("Data received from local database");

        } else
            text.setText("Data do not received from local database of from the server");
    }
